...
def photon_rocket_fuel_fraction(
    thrust_time: float, acceleration: float = 9.80665, efficiency: float = 0.5
) -> float:
    """
    ...
    Parameters:
        ...
        efficiency : float
            ...
            Default = 0.5 (50%, more realistic for advanced designs).
...
if __name__ == "__main__":
    ...
    t = photon_rocket_accel_time(fuel_mass, dry_mass, efficiency=0.5)
    years = t / 60.0 / 60.0 / 24.0 / 365.25
    print(f"Dry mass {dry_mass:.0f}, fuel mass {fuel_mass:.0f} means {years:.2f} years at 1g with 50% efficiency")

    # Calculate propellant fraction for 3.52 years of 1g acceleration at 50% efficiency
    years = 3.52
    secs = 365.25 * 86400 * years
    f = photon_rocket_fuel_fraction(secs, 9.80665, 0.5)
    print(
        f"Propellant mass fraction: {f * 100:.4f}% for {years} years of 1g acceleration at 50% efficiency"
    )
