...
/// <summary>
///     A struct representing a fraction of the speed of light, intentionally not convertible to a double. No implicit constructor here so we check its
...
internal readonly record struct FractionOfC
{
	...
	/// </summary>
	public FractionOfC(double fraction) =>
		Value = Math.Abs(fraction) < 1.0 ? fraction : throw new ArgumentException("Fraction of C must be less than 1.0");
...
internal static class Tools
{
	...
	public static Speed CheckVelocity(Speed velocity) => Math.Abs(velocity.MetersPerSecond) < C_MPS
		? velocity
		: throw new ArgumentException($"Speed must be less than C: {velocity.MetersPerSecond} metres per second");

	/// <summary>
	///     Calculate relativistic acceleration for a given constant acceleration and time
	/// </summary>
	public static Speed RelativisticAcceleration(Acceleration acceleration, Duration time) =>
		Speed.FromMetersPerSecond(C_MPS * Math.Tanh(acceleration.MetersPerSecondSquared * time.Seconds / C_MPS));
	...
	/// </summary>
	/// <param name="mass">Mass of the Earth</param>
	/// <param name="radius">Radius of the Earth</param>
	...
	public static (Duration time, Speed velocity) FallTimeAndVelocity(Mass mass, Length radius, Length altitude)
	{
		// Initial and final positions
		var r0 = radius.Meters + altitude.Meters; // initial
		var rf = radius.Meters; // final
		...
		var ratio = rf / r0;
		var t = Math.Sqrt(Math.Pow(r0, 3) / (2.0 * G * mass.Kilograms))
				* (Math.Acos(Math.Sqrt(ratio)) + Math.Sqrt(ratio * (1.0 - ratio)));

		// Final velocity at impact using energy conservation:
		// v = sqrt(2GM * (1/rf - 1/r0))
		var v = Math.Sqrt(2 * G * mass.Kilograms * ((1.0 / rf) - (1.0 / r0)));
