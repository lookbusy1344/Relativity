...
describe("formatSignificant", () => {
	...
	describe("Very large numbers", () => {
		it("should handle extremely large positive numbers in decimal notation", () => {
			...
			expect(rl.formatSignificant(new Decimal("9007199254740992.123456"))).toBe(
				"9,007,199,254,740,992.12"
			);
			expect(rl.formatSignificant(new Decimal("12345678901234567890.987654321"))).toBe(
				"12,345,678,901,234,567,890.99"
			); // rounds up

			// Even larger numbers - now formatted in decimal notation, not scientific
			expect(rl.formatSignificant(new Decimal("999999999999999999999999999999.123456789"))).toBe(
				"999,999,999,999,999,999,999,999,999,999.12"
			);
			expect(
				rl.formatSignificant(new Decimal("1234567890123456789012345678901234567890.5555555"))
			).toBe("1,234,567,890,123,456,789,012,345,678,901,234,567,890.56"); // rounds up
		});
		...
		it("should handle numbers with many integer digits and various decimal patterns", () => {
			...
			expect(rl.formatSignificant(new Decimal("123456789012345678901234567890.000001"))).toBe(
				"123,456,789,012,345,678,901,234,567,890"
			);
			expect(rl.formatSignificant(new Decimal("123456789012345678901234567890.999999"))).toBe(
				"123,456,789,012,345,678,901,234,567,891"
			); // rounds up to next integer
			expect(rl.formatSignificant(new Decimal("123456789012345678901234567890.123456"))).toBe(
	...
	describe("Very small numbers", () => {
		it("should handle extremely small positive numbers in decimal notation", () => {
			...
			expect(rl.formatSignificant(new Decimal("0.000000123456789"), "", 20)).toBe(
				"0.000000123456789"
			);
	...
	describe("Extreme number handling", () => {
		it("should format extremely large numbers in decimal notation", () => {
			...
			expect(result2).toContain(
				"123,456,000,000,000,000,000,000,000,000,000,000,000,000,000,000,000"
			);
		});
		...
		it("should format extremely small numbers in decimal notation", () => {
			...
			expect(rl.formatSignificant(smallNum, "", 105)).toContain(
	...
	describe("Different significantDecimalPlaces values", () => {
		...
		it("should handle 20 decimal places with large numbers", () => {
			...
			expect(
				rl.formatSignificant(new Decimal("12345678901234567890.12345678901234567890123456"), "", 20)
	...
	describe("ignoreChar functionality", () => {
		...
		it("should handle single character ignoreChar requirement", () => {
			...
			expect(() => rl.formatSignificant(new Decimal("123.456"), "ab", 2)).toThrow(
				"ignoreChar must be a single character or empty"
			);
			expect(() => rl.formatSignificant(new Decimal("123.456"), "99", 2)).toThrow(
				"ignoreChar must be a single character or empty"
			);
		});
	});
	...
	describe("Stress tests with extreme values", () => {
		it("should handle numbers with 50+ decimal digits", () => {
			...
			expect(rl.formatSignificant(longDecimal, "", 50)).toBe(
	...
	describe("Real-world physics calculations", () => {
		...
		it("should handle velocities extremely close to c in fraction form", () => {
			...
			expect(rl.formatSignificant(v2, "9", 10)).toBe(
		...
		it("should handle Planck length", () => {
			...
			expect(rl.formatSignificant(planckLength, "", 40)).toBe(
				"0.000000000000000000000000000000000016162"
			);
		});
...
describe("Calc Tab Operations", () => {
	...
	describe("pionRocketAccelTime", () => {
		...
		it("should increase time with lower efficiency", () => {
			...
			const time70 = rl.pionRocketAccelTime(1000, 1000, 0.7);
		...
		it("should handle Decimal inputs", () => {
			const time = rl.pionRocketAccelTime(new Decimal(3000), new Decimal(1500), new Decimal(0.85));
			expect(time.isFinite()).toBe(true);
			expect(time.gt(0)).toBe(true);
		});
	...
	describe("pionRocketFuelFraction", () => {
		...
		it("should decrease fuel fraction with higher efficiency", () => {
			const thrustTime = new Decimal(365).mul(60 * 60 * 24); // 1 year
			const fraction70 = rl.pionRocketFuelFraction(thrustTime, rl.g, 0.7);
	...
	describe("pionRocketFuelFractionsMultiple", () => {
		...
		it("should return results as percentages (multiplied by 100)", () => {
			...
			const fractions = rl.pionRocketFuelFractionsMultiple(thrustTime, rl.g, [0.85]);
		...
		it("should handle many efficiency values", () => {
			...
			const fractions = rl.pionRocketFuelFractionsMultiple(thrustTime, rl.g, efficiencies);
...
describe("Spacetime Interval and Four-Momentum Functions", () => {
	...
	describe("spacetimeInterval3d", () => {
		...
		it("equals 1d interval when dy=dz=0", () => {
			...
			const event2_3d: [Decimal, Decimal, Decimal, Decimal] = [
				dt,
				dx,
				new Decimal(0),
				new Decimal(0),
	...
	describe("invariantMassFromEnergyMomentum", () => {
		it("calculates invariant mass from energy and momentum", () => {
			...
			const expected = energy
				.div(rl.c.pow(2))
				.pow(2)
				.sub(momentum.div(rl.c.pow(2)).pow(2))
				.sqrt();
